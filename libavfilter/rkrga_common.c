                              ...
                              AVFrame *picref, int do_overlay, int pat_preproc)
{
    ...
    frame_list = nb_link ?
        (pat_preproc ? &r->pat_preproc_in_frame_list : &r->pat_frame_list) : &r->src_frame_list;
    ...
    if (is_afbc) {
        ...
        if (desc->layers[0].planes[0].offset > 0) {
            afbc_offset_y = desc->layers[0].planes[0].offset / w_stride;
            info.rect.yoffset += afbc_offset_y;
        }
        if (in_info->pix_fmt == AV_PIX_FMT_NV12 ||
                             ...
                             const AVFrame *in, int pat_preproc)
{
    ...
    frame_list = pat_preproc ? &r->pat_preproc_out_frame_list : &r->dst_frame_list;
    ...
    if ((ret = av_hwframe_get_buffer(hw_frame_ctx, out_frame->frame, 0)) < 0) {
        av_log(ctx, AV_LOG_ERROR, "Cannot allocate a surface: %d\n", ret);
        return NULL;
    }
...
static av_cold int init_hwframes_ctx(AVFilterContext *avctx)
{
    ...
    hwfc_out->width     = FFALIGN(outlink->w, 16);
    hwfc_out->height    = FFALIGN(outlink->h, 16);
...
static av_cold int init_pat_preproc_hwframes_ctx(AVFilterContext *avctx)
{
    ...
    hwfc_pat->width     = FFALIGN(inlink0->w, 16);
    hwfc_pat->height    = FFALIGN(inlink0->h, 16);
...
static av_cold int fill_rga_frame_info_by_link(RGAFrameInfo *info,
                                               AVFilterLink *link,
                                               int nb_link, int is_inlink)
{
    ...
    if (!map_av_to_rga_format(hwfc->sw_format, &info->rga_fmt, (is_inlink && nb_link > 0))) {
        av_log(NULL, AV_LOG_ERROR, "Unsupported %s pad %d format: %s\n",
...
av_cold int ff_rkrga_init(AVFilterContext *avctx, RKRGAParam *param)
{
    ...
    for (i = 0; i < avctx->nb_inputs; i++) {
        ret = fill_rga_frame_info_by_link(&r->in_rga_frame_infos[i], avctx->inputs[i], i, 1);
        if (ret < 0)
            goto failed;
    }
    ...
    if (avctx->nb_inputs > 1) {
        ...
        r->in_rga_frame_infos[0].blend_mode =
            (r->in_rga_frame_infos[1].pix_desc->flags & AV_PIX_FMT_FLAG_ALPHA) ? 0x504 : 0x501;

        if (param->in_global_alpha > 0 && param->in_global_alpha <= 255)
            r->in_rga_frame_infos[0].blend_mode ^= (param->in_global_alpha << 16);
        else
            r->in_rga_frame_infos[0].blend_mode ^= 0xff0000; /* 255 << 16 */
    ...
    /* OUT RGAFrameInfo */
    ret = fill_rga_frame_info_by_link(&r->out_rga_frame_info, avctx->outputs[0], 0, 0);
    if (ret < 0)
        goto failed;
...
av_cold int ff_rkrga_close(AVFilterContext *avctx)
{
    ...
    clear_frame_list(&r->pat_preproc_in_frame_list);
    clear_frame_list(&r->pat_preproc_out_frame_list);
