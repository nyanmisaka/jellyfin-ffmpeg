...
#define CONSTANT_C(a, b, c, d) \
    ...
    switch (s->tonemap_mode) {
        ...
        case TONEMAP_MODE_RGB:
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_tm, s->cu_module, d ? "tonemap_rgb_d" : "tonemap_rgb"));
            if (ret < 0) goto fail2;
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_dovi, s->cu_module,
                                                   s->tradeoff == 1 ? (d ? "tonemap_dovi_rgb_d_f" : "tonemap_dovi_rgb_f")
                                                                    : (d ? "tonemap_dovi_rgb_d"   : "tonemap_dovi_rgb")));
            if (ret < 0) goto fail2;
            break;
        case TONEMAP_MODE_LUM:
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_tm, s->cu_module, d ? "tonemap_lum_d" : "tonemap_lum"));
            if (ret < 0) goto fail2;
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_dovi, s->cu_module,
                                                   s->tradeoff == 1 ? (d ? "tonemap_dovi_lum_d_f" : "tonemap_dovi_lum_f")
                                                                    : (d ? "tonemap_dovi_lum_d"   : "tonemap_dovi_lum")));
            if (ret < 0) goto fail2;
            break;
        case TONEMAP_MODE_ITP:
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_tm, s->cu_module, d ? "tonemap_itp_d" : "tonemap_itp"));
            if (ret < 0) goto fail2;
            ret = CHECK_CU(cu->cuModuleGetFunction(&s->cu_func_dovi, s->cu_module,
                                                   s->tradeoff == 1 ? (d ? "tonemap_dovi_itp_d_f" : "tonemap_dovi_itp_f")
                                                                    : (d ? "tonemap_dovi_itp_d"   : "tonemap_dovi_itp")));
            if (ret < 0) goto fail2;
            break;
    }
