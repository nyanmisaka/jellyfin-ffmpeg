...
void map_four_pixels_rgb(float4 *r4, float4 *g4, float4 *b4, float peak) {
    ...
    sig.x = TONE_FUNC(sig.x, src_peak_delin_pq, dst_peak_delin_pq);
    sig.y = TONE_FUNC(sig.y, src_peak_delin_pq, dst_peak_delin_pq);
    sig.z = TONE_FUNC(sig.z, src_peak_delin_pq, dst_peak_delin_pq);
    sig.w = TONE_FUNC(sig.w, src_peak_delin_pq, dst_peak_delin_pq);
#else
    sig.x = TONE_FUNC(sig.x, peak, 1.0f);
    sig.y = TONE_FUNC(sig.y, peak, 1.0f);
    sig.z = TONE_FUNC(sig.z, peak, 1.0f);
    sig.w = TONE_FUNC(sig.w, peak, 1.0f);
#endif
    sig = fmin(sig, 1.0f);

    float4 factor = sig / sig_old;
    *r4 *= factor;
    *g4 *= factor;
    *b4 *= factor;
}
