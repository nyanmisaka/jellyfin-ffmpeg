...
static __inline__ __device__
float3 map_one_pixel_rgb_mode_lum(float3 rgb, const FFCUDAFrame& src, const FFCUDAFrame& dst) {
...
static __inline__ __device__
float3 map_to_src_space_from_yuv_dovi_fast(float3 yuv) {
    float3 c = ycc2rgb(yuv.x, yuv.y, yuv.z);
    c = lms2rgb_fast(c.x, c.y, c.z);
    return c;
}
...
#define _YCC2RGB_FS \
    float3 c0 = map_to_src_space_from_yuv_dovi_fast(yuv0); \
    float3 c1 = map_to_src_space_from_yuv_dovi_fast(yuv1); \
    float3 c2 = map_to_src_space_from_yuv_dovi_fast(yuv2); \
    float3 c3 = map_to_src_space_from_yuv_dovi_fast(yuv3);
...
#define _TONEMAP_LUM \
    c0 = map_one_pixel_rgb_mode_lum(c0, src, dst); \
    c1 = map_one_pixel_rgb_mode_lum(c1, src, dst); \
    c2 = map_one_pixel_rgb_mode_lum(c2, src, dst); \
    c3 = map_one_pixel_rgb_mode_lum(c3, src, dst);

#define _TONEMAP_ITP \
    c0 = map_one_pixel_itp_mode(c0, src, dst); \
    c1 = map_one_pixel_itp_mode(c1, src, dst); \
    c2 = map_one_pixel_itp_mode(c2, src, dst); \
    c3 = map_one_pixel_itp_mode(c3, src, dst);
...
#define _RGB2YUV_FS \
    ...
    c3 = clamp3(lrgb2lrgb(c3), 0.0f, 1.0f); \
    yuv0 = lrgb2yuv(c0); \
    yuv1 = lrgb2yuv(c1); \
    yuv2 = lrgb2yuv(c2); \
    yuv3 = lrgb2yuv(c3);
