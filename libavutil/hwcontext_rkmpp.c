...
static void rkmpp_device_free(AVHWDeviceContext *hwdev)
{
    AVRKMPPDeviceContext *hwctx = hwdev->hwctx;
    close(hwctx->fd);
}

static int rkmpp_check_drm_device(AVHWDeviceContext *hwdev, const char *device)
{
    AVRKMPPDeviceContext *hwctx = hwdev->hwctx;
    ...
    if (strstr(version->name, "rockchip") ||
        strstr(version->name, "rk")) {
        av_log(hwdev, AV_LOG_INFO, "Opened Rockchip DRM device %s: "
               "driver %s version %d.%d.%d.\n", device, version->name,
               version->version_major, version->version_minor,
               version->version_patchlevel);
    } else {
        av_log(hwdev, AV_LOG_VERBOSE, "Skipping non-Rockchip DRM device %s: "
               "driver %s version %d.%d.%d.\n", device, version->name,
               version->version_major, version->version_minor,
               version->version_patchlevel);
        drmFreeVersion(version);
    ...
    if (drmGetCap(hwctx->fd, DRM_CAP_DUMB_BUFFER, &has_dumb) < 0 ||
        !has_dumb) {
        av_log(hwdev, AV_LOG_ERROR, "Opened Rockchip DRM device "
                               ...
                               AVDictionary *opts, int flags)
{
    ...
    int err, n, max_devices = 8;
 
    if (device) {
        err = rkmpp_check_drm_device(hwdev, device);
        if (err)
            return err;
    } else {
        for (n = 0; n < max_devices; n++) {
            snprintf(drm_dev, sizeof(drm_dev), DRM_DEV_NAME, DRM_DIR_NAME, n);
            device = drm_dev;
    ...
    hwctx->flags = AV_RKMPP_FLAG_BO_DMA32 | AV_RKMPP_FLAG_BO_CACHABLE;

    opt_d = av_dict_get(opts, "dma32", NULL, 0);
    if (opt_d && !strtol(opt_d->value, NULL, 10))
        hwctx->flags &= ~AV_RKMPP_FLAG_BO_DMA32;

    opt_d = av_dict_get(opts, "cacheable", NULL, 0);
    if (opt_d && !strtol(opt_d->value, NULL, 10))
        hwctx->flags &= ~AV_RKMPP_FLAG_BO_CACHABLE;

    hwdev->free = &rkmpp_device_free;
...
static int rkmpp_get_padded_bits_per_pixel(enum AVPixelFormat pix_fmt)
{
    const AVPixFmtDescriptor *pixdesc = av_pix_fmt_desc_get(pix_fmt);

    return pix_fmt == AV_PIX_FMT_NV15 ? 16 :
...
static AVBufferRef *rkmpp_drm_pool_alloc(void *opaque, size_t size)
{
    ...
    if (ret < 0) {
        if (dmcb.flags & AV_RKMPP_FLAG_BO_DMA32) {
            dmcb.flags &= ~AV_RKMPP_FLAG_BO_DMA32;
            ret = drmIoctl(hwctx->fd, DRM_IOCTL_MODE_CREATE_DUMB, &dmcb);
        }
        if (ret < 0) {
            av_log(hwfc, AV_LOG_ERROR,
                   "Failed to create DRM dumb<w,h,bpp: %d,%d,%d>: %m.\n",
    ...
    ret = drmPrimeHandleToFD(hwctx->fd, dmcb.handle, DRM_CLOEXEC | DRM_RDWR,
                             &desc->objects[0].fd);
    if (ret) {
        av_log(hwfc, AV_LOG_ERROR, "Failed to convert DRM handle to fd: %m\n");
        goto fail;
    }
    ...
    if (ret) {
        av_log(hwfc, AV_LOG_ERROR, "Failed to map DRM dumb: %m\n");
        goto fail;
    }

    // default read and write
    desc->objects[0].ptr = mmap(NULL, dmcb.size, PROT_READ | PROT_WRITE,
                                MAP_SHARED, hwctx->fd, dmmd.offset);
    if (desc->objects[0].ptr == MAP_FAILED) {
        av_log(hwfc, AV_LOG_ERROR, "Failed to mmap: %m\n");
        goto fail;
    }
