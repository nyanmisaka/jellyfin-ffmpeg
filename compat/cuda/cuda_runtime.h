...
template<>
static __inline__ __device__ float4 tex2D<float4>(cudaTextureObject_t texObject, float x, float y)
{
    float4 ret;
...
template<>
static __inline__ __device__ float tex2D<float>(cudaTextureObject_t texObject, float x, float y)
{
    return tex2D<float4>(texObject, x, y).x;
}

template<>
static __inline__ __device__ float2 tex2D<float2>(cudaTextureObject_t texObject, float x, float y)
{
...
static __inline__ __device__ float __sqrtf(float a) { return __nvvm_sqrt_approx_f(a); }
...
static __inline__ __device__ float __powf(float a, float b) { return __nvvm_ex2_approx_f(b * __nvvm_lg2_approx_f(a)); }
