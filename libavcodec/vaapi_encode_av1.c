...
static int vaapi_encode_av1_set_tile(AVCodecContext *avctx)
{
    ...
    int max_tile_area_sb, widest_tile_sb;
    ...
    if ((priv->sb_cols + tile_width_sb - 1) / tile_width_sb == priv->tile_cols) {
        ...
        for (i = 0; i < priv->tile_cols; i++) {
            ...
            widest_tile_sb = FFMIN(widest_tile_sb, priv->width_in_sbs_minus_1[i] + 1);
        }
    }

    if (min_log2_tiles)
        max_tile_area_sb = (priv->sb_rows * priv->sb_cols) >> (min_log2_tiles + 1);
    else
        max_tile_area_sb = priv->sb_rows * priv->sb_cols;
    priv->max_tile_height_sb = FFMAX(1, max_tile_area_sb / widest_tile_sb);

    if (priv->tile_rows != av_clip(priv->tile_rows, (priv->sb_rows + priv->max_tile_height_sb - 1) / priv->max_tile_height_sb, priv->sb_rows)) {
        priv->tile_rows = av_clip(priv->tile_rows, (priv->sb_rows + priv->max_tile_height_sb - 1) / priv->max_tile_height_sb, priv->sb_rows);
        av_log(avctx, AV_LOG_WARNING, "Invalid tile rows, correct to %d.\n", priv->tile_rows);
    }

    priv->tile_rows_log2 = tile_log2(1, priv->tile_rows);
    tile_height_sb = (priv->sb_rows + (1 << priv->tile_rows_log2) - 1) >>
                      priv->tile_rows_log2;

    if ((priv->sb_rows + tile_height_sb - 1) / tile_height_sb == priv->tile_rows) {
        for (i=0; i < priv->tile_rows - 1; i++)
            priv->height_in_sbs_minus_1[i] = tile_height_sb - 1;
        priv->height_in_sbs_minus_1[i] = priv->sb_rows - (priv->tile_rows - 1) * tile_height_sb - 1;
    } else {
        for (i = 0; i < priv->tile_rows; i++)
            priv->height_in_sbs_minus_1[i] = (i + 1) * priv->sb_rows / priv->tile_rows - i * priv->sb_rows / priv->tile_rows - 1;
    }

    if ((priv->sb_cols + tile_width_sb - 1) / tile_width_sb == priv->tile_cols &&
        (priv->sb_rows + tile_height_sb - 1) / tile_height_sb == priv->tile_rows) {
        priv->uniform_tile = 1;
