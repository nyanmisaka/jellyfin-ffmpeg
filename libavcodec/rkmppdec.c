                                   ...
                                   int64_t out_pts)
{
    while (list) {
        if (list->queued == 1) {
            ...
            /* display order, normal & overflow cases */
            if ((in_pts > out_pts && frame->pts <= out_pts) ||
                (in_pts < out_pts && in_pts < frame->pts && frame->pts <= out_pts)) {
...
static SideDataFrame *get_free_sd_frame(SideDataFrame **list)
{
    ...
    if (!out) {
        ...
        if (!out) {
            av_log(NULL, AV_LOG_ERROR, "Cannot alloc new sidedata frame\n");
            return NULL;
        }
        out->queued = 1;
        out->next   = *list;
...
static av_cold int rkmpp_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        if (sd && sd->size >= sizeof(r->dovi_ctx.cfg))
            r->dovi_ctx.cfg = *(AVDOVIDecoderConfigurationRecord *) sd->data;
    }
#endif
...
#if CONFIG_HEVC_RKMPP_DECODER
    /* dequeue & export dovi sidedata for HEVC */
    if (avctx->codec_id == AV_CODEC_ID_HEVC) {
        SideDataFrame *sd_frame = find_sd_frames(r->sd_frame_list, mpp_frame_pts); /* display order */

        if (sd_frame && sd_frame->frame) {
            for (int i = 0; i < sd_frame->frame->nb_side_data; i++) {
                const AVFrameSideData *sd_src = sd_frame->frame->side_data[i];
                AVFrameSideData *sd_dst;

                sd_dst = av_frame_new_side_data(frame, sd_src->type,
                                                sd_src->size);
                if (!sd_dst) {
...
static int rkmpp_send_packet(AVCodecContext *avctx, AVPacket *pkt)
{
    ...
    if (avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        if ((pix_desc->comp[0].depth < 10) &&
            !(pix_desc->flags & AV_PIX_FMT_FLAG_HWACCEL))
            goto exit;

        pix_desc_sw = av_pix_fmt_desc_get(avctx->sw_pix_fmt);
        if (pix_desc_sw && pix_desc_sw->comp[0].depth < 10)
            goto exit;

        /* parse extradata */
        if (!r->parsed_extradata && avctx->extradata && avctx->extradata_size > 0) {
            r->is_nalff = r->nal_length_size = 0;
            memset(&r->h265_ps, 0, sizeof(r->h265_ps));
            memset(&r->h265_sei, 0, sizeof(r->h265_sei));

            ret = ff_hevc_decode_extradata(avctx->extradata, avctx->extradata_size,
                                           &r->h265_ps, &r->h265_sei,
                                           &r->is_nalff, &r->nal_length_size,
        ...
        if (sd && sd_size >= sizeof(r->dovi_ctx.cfg)) {
            int old = r->dovi_ctx.cfg.dv_profile;
            r->dovi_ctx.cfg = *(AVDOVIDecoderConfigurationRecord *) sd;
            if (old)
                av_log(avctx, AV_LOG_DEBUG,
                       "New DOVI configuration record from input packet (profile %d -> %u)\n",
                       old, r->dovi_ctx.cfg.dv_profile);
        }

        /* split packet into NAL units */
        ret = ff_h2645_packet_split(&r->h2645_pkt, pkt->data, pkt->size, avctx,
                                    r->nal_length_size, avctx->codec_id, split_flags);
        if (ret < 0) {
            av_log(avctx, AV_LOG_DEBUG,
        ...
        if (r->h2645_pkt.nb_nals > 1 && r->h2645_pkt.nals[r->h2645_pkt.nb_nals - 1].type == HEVC_NAL_UNSPEC62 &&
            ...
            && !r->h2645_pkt.nals[r->h2645_pkt.nb_nals - 1].temporal_id) {
            H2645NAL *nal = &r->h2645_pkt.nals[r->h2645_pkt.nb_nals - 1];
            if (r->rpu_buf)
                av_buffer_unref(&r->rpu_buf);

            r->rpu_buf = av_buffer_alloc(nal->raw_size - 2);
            if (!r->rpu_buf)
                goto exit;
        ...
        /* queue dovi sidedata to the list */
        {
            ...
            if (r->rpu_buf) {
                AVFrameSideData *rpu;
                rpu = av_frame_new_side_data_from_buf(tmp_frame,
            ...
            if ((ret = ff_dovi_attach_side_data(&r->dovi_ctx, tmp_frame)) < 0) {
                av_frame_free(&tmp_frame);
                goto exit;
            }

            sd_frame = get_free_sd_frame(&r->sd_frame_list);
            if (!sd_frame) {
                av_frame_free(&tmp_frame);
                goto exit;
            }
